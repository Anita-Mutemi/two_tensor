...
from projects.schemas.filters import (
...
class ProjectDescriptionPreprocessor(ArgsPreprocessor):
    def __call__(self, *, project_data: ProjectData, **kwargs) -> dict:
        inputs = {"project_data": project_data, "kwargs": kwargs}

        info_fields = ["location", "team_size"]
        startup_info = {k: v for k, v in project_data.analytics.__dict__.items() if k in info_fields and v}
        startup_info = "\n".join([f"{k}: {v}" for k, v in startup_info.items()])

        description_attrs = [
            getattr(project_data, attr, "") for attr in ["about", "description", "category", "industry", "specialities"]
        ]
        description_attrs = [attr[0] if isinstance(attr, tuple) else attr for attr in description_attrs]
        ...
        valid_descriptions = [re.sub(r"\s+", " ", attr).strip() for attr in description_attrs if attr]

        if not any(valid_descriptions):
            raise FilterPreconditionException("no valid description attributes found", inputs=inputs)

        startup_data = startup_info + "\n" + "\n".join(valid_descriptions)

        if len(startup_data) < 30:
            raise FilterPreconditionException("startup description is too short", inputs=inputs)
...
class FundPreprocessor(ArgsPreprocessor):
    def __call__(self, *, db, signal: FundIdSchema, **kwargs) -> dict:
        inputs = {"signal": signal, "kwargs": kwargs}

        try:
            fund = db.scalars(select(Fund).where(Fund.id == signal.id)).one()
        except NoResultFound:
            raise FilterPreconditionException(f"fund with id {signal.id} not found", inputs=inputs)

        inputs.update(fund_data=fund.to_dict())
        if not fund.thesis:
            raise FilterPreconditionException(f"fund with id {signal.id} ({fund.name}) has no thesis", inputs=inputs)
...
class ProjectFilter(ABC):
    def __init__(self, id: str, display_name: str, none_value_should_pass: bool = True, **kwargs) -> None:
        self.id = id
...
class GptFilter(ProjectFilter):
    ...
    def __call__(self, project_data: ProjectData, **kwargs) -> FilterResult:
        ...
        if not all(key in prompt_kwargs for key in self.required_kwargs):
            raise ValueError(
                "filter is missing kwargs required for prompt formatting."
                "\nmake sure they are either contained in project data or "
                f"provided as **kwargs.\nmissing args: "
                + str([key for key in self.required_kwargs if not key in prompt_kwargs])
                + "\nall args: "
                + str(prompt_kwargs)
            )
        ...
        passed = asyncio.run(chat_completion(prompt, response_validator=parse_bool_response, model=self.model))
...
class RegexFilter(ProjectFilter):
    ...
    def __call__(self, project_data: ProjectData, **kwargs) -> FilterResult:
        ...
        if self.mode == "include":
            ...
            if match is None:
                return FilterResult(filter_config=self.config, passed=False, input=text)
        elif self.mode == "exclude":
            # return false if any excluded words are found
            if match is not None:
                return FilterResult(filter_config=self.config, passed=False, input=text, output=match.groups())

        return FilterResult(filter_config=self.config, passed=True, input=text)
...
class TagsFilter(ProjectFilter):
    ...
    def __call__(self, project_data: ProjectData, **kwargs) -> FilterResult:
        ...
        return FilterResult(filter_config=self.config, passed=passed, input=tags)
...
FILTER_STARTUP_DATA = EXCLUSION_FILTERS + [
    ...
    # founded date
    RangeFilter(
        id="founded_year", display_name="Founded year", field_getter=lambda p: p.analytics.founded, min_value=2016
    ),
    # )
    # )
]
...
def apply_filters(db: Session, *, id: str, name: str, filters: Sequence[ProjectFilter], **kwargs):
