...
from packages.public_api.routers.demo import public_search

from ..dependencies import LoggedInUser, PrivateSession, QueryParams
from packages.public_api.schemas.schemas import SearchFiltersSchema
from packages.public_api.schemas.funds import FundFullSchema
...
def search_funds(session: PrivateSession,
                 _: LoggedInUser,
                 params: QueryParams
                 ) -> list[FundFullSchema]: # type: ignore
    orm_q = select(Fund)
...
def search_endpoint(search_filters: SearchFiltersSchema, _: LoggedInUser, private_s: PrivateSession) -> list[ProjectSchema]:
    ...
    if not project_uuids:
        return []
    projects = public_search.query(Project).filter(Project.uuid.in_(project_uuids)).all()
